// Netlify serverless function — Hub Directory waitlist relay
//
...
//
// The browser talks to /.netlify/functions/hub-waitlist. Only this file talks
// to Google. Nothing about the Apps Script itself changed.
//
// Optional environment variable (Netlify → Site configuration → Environment
// variables): HUB_WAITLIST_URL overrides the default below, useful if the
// Apps Script is redeployed and gets a new /exec URL.

// The Hub waitlist Apps Script Web App. Moved here from js/main.js.
// If you redeploy sep-hub-waitlist-script.gs, paste the new /exec URL here.
const DEFAULT_APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyv6mMc4UDu6elcXeFa3rIuV0mGTc9fYCMLCHXOQ7sa-mhKtEOo8Wu3WB-IoYw0CtEZVg/exec';
...
// already_registered, which the site shows as the normal confirmation. Slow
// for that one visitor, but the row is saved and the message is correct.
...
exports.handler = async function (event) {
  ...
  let firstName, lastName, email, phone, category;
  try {
  ...
  const appsScriptUrl = process.env.HUB_WAITLIST_URL || DEFAULT_APPS_SCRIPT_URL;

  // Rebuild the payload from the five known fields. Forwarding event.body
  // as-is would let anyone use this function to post anything they like to
  // the Google endpoint.
  const payload = JSON.stringify({ firstName, lastName, email, phone, category });
  ...
  try {
    ...
    try {
      ...
      console.error('Hub waitlist: non-JSON response from Apps Script', response.status, text.slice(0, 300));
      return { statusCode: 502, headers: CORS_HEADERS, body: JSON.stringify({ result: 'error', message: 'Unexpected response from the waitlist service' }) };
    }

    if (data.result === 'error') {
      console.error('Hub waitlist: Apps Script returned an error', data.message);
    }
    ...
    if (err.name === 'AbortError') {
      console.error('Hub waitlist: Apps Script timed out after ' + TIMEOUT_MS + 'ms');
      return { statusCode: 504, headers: CORS_HEADERS, body: JSON.stringify({ result: 'error', message: 'The waitlist service took too long to answer. Please try again.' }) };
    }
    console.error('Hub waitlist: fetch error', err);
